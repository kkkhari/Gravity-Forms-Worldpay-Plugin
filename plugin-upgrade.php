...
class RGWorldPayUpgrade {

    public static function set_version_info($version_info){
        if ( function_exists('set_site_transient') )
            set_site_transient("gforms_worldpay_version", $version_info, 60*60*12);
        else
            set_transient("gforms_worldpay_version", $version_info, 60*60*12);
    }

    public static function check_update($plugin_path, $plugin_slug, $plugin_url, $offering, $key, $version, $option){
        $version_info = function_exists('get_site_transient') ? get_site_transient("gforms_worldpay_version") : get_transient("gforms_worldpay_version");
    ...
    public static function display_changelog($offering, $key, $version){
        ...
        if ( is_wp_error( $raw_response ) || 200 != $raw_response['response']['code']){
            $page_text = sprintf(__("Oops!! Something went wrong.%sPlease try again or %scontact us%s.", 'gravityformspaypal'), "<br/>", "<a href='http://www.gravityforms.com'>", "</a>");
        }
        else{
    ...
    public static function get_version_info($offering, $key, $version){
        ...
        // wp_die( es_preit( array( 'raw response is', $raw_response, $url ), false ) );
        if ( is_wp_error( $raw_response ) || 200 != $raw_response['response']['code'])
            return -1;
        else
        {
